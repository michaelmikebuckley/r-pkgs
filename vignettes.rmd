...
* Vignette: this contains a special block of metadata needed by R. Here, you can 
  see the legacy of LaTeX vignettes: the metadata are look like LaTeX commands. 
...
Markdown isn't as powerful as LaTeX, reStructuredText or docbook, but it's simple, easy to write, and easy to read even when it's not rendered. I find markdown' constraints helpful for writing because it lets me focus on the content, and prevents me from messing around with the styling.
...
Once you start using knitr, you'll never look back. Because your code always run when you build the vignette, you can rest assured knowing that all your code works. There's no way for your input and output to be out of sync.
...
* `error = TRUE` captures any errors in the block and show them inline. 
 ...
 prevents the code from being included in the summary R document, which is 
 important because that file must be able to be sourced without error.
...
You can build all vignettes from the console with `devtools::build_vignettes()`, but this is rarely useful. Instead us `devtools::build()` to create a package bundle with the vignettes included.  RStudio's "Build & reload" does not build vignettes to save time. Similarly, `devtools:install_github()` (and friends) will not build vignettes by default because they're time consuming and may require additional packages. You can force building with `devtools:install_github(build_vignettes = TRUE)`. This will also install all suggested packages.
...
Writing a vignette also makes a nice break from coding. In my experience, writing uses a different part of the brain from programming, so if you're sick of programming, trying writing for a bit. (This is related to the idea of [structured programming](http://www.structuredprocrastination.com/).).
...
If you write a nice vignette, consider submitting it to the Journal of Statistical Software or the R Journal. Both journals are electronic only and peer-reviewed. The latter can be very helpful for improving the quality of your vignette and the related software.
